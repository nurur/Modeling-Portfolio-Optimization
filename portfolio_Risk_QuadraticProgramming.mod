#NURUR RAHMAN
#Minimizing Portfolio Risk : Quadratic Programming Formulation
#
#
...
#Objective function 
minimize portfolioRisk : sum{i in 1..A, j in 1..A} (Q[i,j]*W[i]*W[j]) ;
...
# Number of assets available for investment is at most 12
...
subject to cardinality2 {i in 1..A}: W[i] <= 1.0*Y[i];


# Maximum amount of an asset that can be used in the portfolio
# Any value close to zero 
subject to cardinality3 {i in 1..A}: W[i] >= 0.0*Y[i];
